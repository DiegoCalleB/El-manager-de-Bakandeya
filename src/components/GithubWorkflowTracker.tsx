...
export default function GithubWorkflowTracker({
  ...
  const renderStatusBadge = (status: string, conclusion: string | null) => {
    if (status === 'queued') {
      return (
        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-mono font-bold bg-amber-500/10 text-amber-500 border border-amber-500/20 animate-pulse">
          En Cola
        </span>
      );
    }
    if (status === 'in_progress') {
      return (
        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-mono font-bold bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 animate-pulse">
          Corriendo
        </span>
      );
    }
    if (status === 'completed') {
      if (conclusion === 'success') {
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-mono font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
            ✓ Éxito
          </span>
        );
      }
      if (conclusion === 'failure') {
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-mono font-bold bg-rose-500/10 text-rose-400 border border-rose-500/20">
            ✗ Falló
          </span>
        );
      }
      return (
        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-mono font-bold bg-neutral-800 text-neutral-400 border border-neutral-700">
          {conclusion || 'Terminado'}
        </span>
      );
    }
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-mono bg-neutral-900 text-neutral-500 border border-neutral-800">
        {status}
      </span>
    );
  };
  ...
  const renderAgentBadge = (agent: string | undefined) => {
    ...
    if (agent === 'Lector') badgeClass = 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20';
  ...
  return (
    <div className={`mt-6 border-t border-dashed ${isStitchLight ? 'border-slate-200' : 'border-neutral-800'} pt-6 space-y-4`}>
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {isDemo && (
            <span className="text-[9px] font-mono text-[#f2ca50]/80 bg-[#f2ca50]/5 border border-[#f2ca50]/20 px-2 py-0.5 rounded">
              MODO SIMULADOR
            </span>
          )}
          <button
            ...
            disabled={loading}
            className={`p-1.5 rounded-lg border flex items-center justify-center transition-all cursor-pointer hover:scale-105 active:scale-95 ${
              isStitchLight 
      ...
      {error ? (
        ...
        <div className="p-4 rounded-xl border border-dashed text-center space-y-2 text-neutral-500 border-neutral-800 bg-neutral-950/20">
            ...
            return (
              <div 
                ...
                className={`border rounded-xl transition-all overflow-hidden ${
                  isStitchLight 
                    ? `bg-slate-50 border-slate-200/80 ${isExpanded ? 'ring-1 ring-indigo-500/20' : ''}` 
                    : `bg-neutral-950/40 border-neutral-800/80 ${isExpanded ? 'border-neutral-700 bg-neutral-950/80' : ''}`
                }`}
              >
                {/* Run Main Row */}
                <div 
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    {/* Actor avatar / icon */}
                    <div className="w-6 h-6 rounded-lg overflow-hidden border border-neutral-800 bg-neutral-900 shrink-0 flex items-center justify-center">
                ...
                {isExpanded && (
                  <div className={`px-4 pb-3 pt-1 border-t ${isStitchLight ? 'border-slate-200 bg-white' : 'border-neutral-900 bg-neutral-950/80'} font-mono`}>
                    ...
                    {isJobLoading ? (
                      ...
                      <div className="space-y-1.5 border-l border-neutral-800 ml-1.5 pl-3 py-1">
