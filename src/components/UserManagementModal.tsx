  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
      ...
      >
        ...
        {/* Tab Selection */}
        <div className={`px-6 pt-3 border-b flex gap-2 ${
          isStitchLight ? 'border-slate-200 bg-slate-50/50' : 'border-neutral-800/80 bg-neutral-900/40'
        }`}>
          <button
            onClick={() => { setActiveTab('list'); setError(null); setSuccessMsg(null); }}
            className={`px-4 py-2 text-xs font-bold font-mono uppercase tracking-wider border-b-2 transition-all flex items-center gap-1.5 ${
              activeTab === 'list'
              ...
              {users.map((u) => {
                ...
                return (
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            {isLeader ? (
                              <span className="px-2 py-0.5 text-[9px] font-mono font-bold uppercase rounded bg-amber-500/15 text-amber-400 border border-amber-500/30 flex items-center gap-1">
                                <Shield className="w-2.5 h-2.5" />
                                <span>Admin</span>
                              </span>
                            ) : (
                              <span className="px-2 py-0.5 text-[9px] font-mono font-bold uppercase rounded bg-blue-500/15 text-blue-400 border border-blue-500/30">
                                Miembro
                              </span>
                            )}
                            {isSelf && (
                              <span className="px-1.5 py-0.5 text-[9px] font-mono rounded bg-emerald-500/20 text-emerald-400">
                                Tú
                              </span>
                            )}
                          </div>
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <select
                          ...
                          className={`px-2 py-1.5 rounded-lg text-xs font-mono font-bold border outline-none cursor-pointer transition-all ${
                            u.role === 'leader'
                              ? 'bg-amber-500/15 text-amber-300 border-amber-500/40 hover:bg-amber-500/25'
                              : 'bg-neutral-900 text-blue-300 border-blue-500/30 hover:bg-neutral-800'
                        ...
                        <button
                          ...
                          className="px-2.5 py-1.5 rounded-lg border border-neutral-700/80 text-xs font-mono hover:bg-neutral-800 text-neutral-300 transition-colors flex items-center gap-1 cursor-pointer"
                        >
                    ...
                    {isEditingThisUser && (
                      <div className="mt-3 pt-3 border-t border-neutral-800/80 flex items-center gap-2 animate-in fade-in duration-200">
                        <input
                          type="text"
                          ...
                          className={`flex-1 px-3 py-1.5 rounded-lg border text-xs font-mono outline-none ${
                            isStitchLight ? 'bg-white border-slate-200' : 'bg-neutral-900 border-neutral-700'
                          }`}
                        />
                        <button
                          type="button"
            ...
            <form onSubmit={handleCreateUser} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    placeholder="Ej: Pablo (Saxo / Vientos)"
                    className={`w-full px-3 py-2 rounded-xl border text-xs outline-none ${
                      isStitchLight ? 'bg-slate-50 border-slate-200' : 'bg-neutral-950 border-neutral-800'
                    }`}
                    required
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="Ej: Saxofón / Vientos, Trombón, Técnico de Sonido"
                  className={`w-full px-3 py-2 rounded-xl border text-xs outline-none ${
                    isStitchLight ? 'bg-slate-50 border-slate-200' : 'bg-neutral-950 border-neutral-800'
                  }`}
                />
                  ...
                  {colors.map((c) => (
                    <button
                      ...
                      className={`w-7 h-7 rounded-full border-2 transition-transform ${
                        newAvatarColor === c ? 'scale-110 border-white ring-2 ring-emerald-500' : 'border-transparent opacity-75 hover:opacity-100'
                      }`}
        ...
        <div className={`px-6 py-3 border-t text-right ${
          ...
          <button
            onClick={onClose}
            className="px-4 py-1.5 rounded-lg text-xs font-mono border border-neutral-700 text-neutral-300 hover:bg-neutral-800 transition-colors"
          >
            Cerrar Panel
          </button>
        </div>
